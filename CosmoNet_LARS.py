...
class CosmoNet:
    ...
    def get_lars_optimizer(self,opt_type,loss,global_step,learning_rate,momentum=0.9,LARS_mode="clip",LARS_eta=0.002,LARS_epsilon=1./16000.):
        ...
        for idx, (g, v) in enumerate(gs_and_vs):
            if g is not None:
                ...
                #multiply gradients
                gs_and_vs[idx] = (math_ops.scalar_mul(lars_local_lr, g), v)
    ...
    def optimize(self, num_nodes, global_step):
        loss = self.loss()
        opt_type = 'ADAM_LARS' #'LARS'
        print("optimizer type is ", opt_type)
        if opt_type =='LARS': 
            with tf.name_scope('LARS_optimizer'):
                ...
                with tf.control_dependencies(update_ops):
                    ...
                    #print("LR_warmup_epoch_num=", hp.RUNPARAM['LR_warmup_epoch_num'])
                    #print("num_batches_per_epoch", num_batches_per_epoch)
                    #print("batch_size=", hp.Input['BATCH_SIZE'])
                    #print("num_nodes=",num_nodes)
                    num_steps_for_warmup = int(num_batches_per_epoch * hp.RUNPARAM['LR_warmup_epoch_num'])
                    #print("num_steps_for_warmup=", num_steps_for_warmup)

                    #print("number of steps for decay=", decay_steps)
                    ...
                    def learning_rate_fn(global_step):
                        ...
                        def b2(): return tf.train.polynomial_decay(
                            base_LR,
                            global_step,
                            decay_steps,
                    ...
                    train_step = self.get_lars_optimizer("SGD",loss,global_step, self.learning_rate, momentum=0.9)
            ...
            with tf.name_scope('ADAM_LARS_optimizer'):
                ...
                with tf.control_dependencies(update_ops):
                    ...
                    def learning_rate_fn(global_step):
                        global_step_val = math_ops.cast(global_step, tf.float32)
                        def b1(): return  (global_step_val * base_LR + (num_steps_for_warmup - global_step_val) * warmup_start_LR)/num_steps_for_warmup
                        def b2(): return  base_LR
                        return tf.cond(tf.less(global_step_val, num_steps_for_warmup), lambda: b1(), lambda: b2())
                    self.learning_rate = learning_rate_fn(global_step)

                    #self.learning_rate = tf.Print(self.learning_rate, [self.learning_rate], message="Current global LR out =")
                    train_step = self.get_lars_optimizer("Adam",loss,global_step, self.learning_rate)
    ...
    def train(self):
        ...
        train_step, loss, lossL1Train,train_true,train_predict = self.optimize(mc.get_nranks(), global_step)
