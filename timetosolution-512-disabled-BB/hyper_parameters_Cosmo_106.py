...
RUNPARAM={
	"num_epoch": 120,              #each epoch means a fully pass over the data. The program might stop before running num_epoch (see next line).        
...
## BB dir
main_dir = someVariable = (os.environ['DW_PERSISTENT_STRIPED_CosmoFlow3'])
target_dir = "/5000"
...
Path={
	...
	"init_data" :  '.',                 #Path where the init data is
        "Model_path" :  './result-106epochs/',                 #Path to save the best model where the validation error is the smallest. And then we use this model for test
        "train_data" : main_dir + target_dir + '/train/',            #path where the  train data is
	"train_result" :  './result-106epochs/',        #path to store the train result
	"val_data" : main_dir + target_dir + '/val/',              #path where the  validation data is
	"val_result" :  './result-106epochs/',          #path to st/data0/jamesarnemann/cosmoNet/' + target_dir + '/result/'ore the validation result
	"test_data" : main_dir + target_dir + '/test/',              #path where the  test data is
	"test_result" : './result-106epochs/',           #path to store the test result
