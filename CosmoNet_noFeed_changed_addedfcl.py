...
#	self.b['b_conv6'] = bias_variable([128])
	...
	self.W['W_fc2'] = weight_variable([1024,512],'w9')
        self.b['b_fc2'] = bias_variable([512])

	self.W['W_fc3'] = weight_variable([512,256],'w10')
        self.b['b_fc3'] = bias_variable([256])
        

	self.W['W_final'] = weight_variable([256,hp.DATAPARAM['output_dim']],'wfinal')
        self.b['b_final'] = bias_variable([hp.DATAPARAM['output_dim']])
...
#            h_conv6 = lrelu(self.BatchNorm(tf.nn.conv3d(h_conv5, self.W['W_conv6'],strides = [1,1,1,1,1],padding = 'VALID') + self.b['b_conv6'],IS_TRAINING=IS_TRAINING,scope = scope+str(6),reuse=reuse),hp.Model['LEAK_PARAMETER'])
        ...
        with tf.name_scope('fc3'):
            h_fc3 = lrelu(tf.matmul(tf.nn.dropout(h_fc2,keep_prob), self.W['W_fc3']) + self.b['b_fc3'],hp.Model['LEAK_PARAMETER'])
	    print 'hfc3', h_fc3.shape
            


        with tf.name_scope('final'):
            h_final = tf.matmul(tf.nn.dropout(h_fc3,keep_prob), self.W['W_final']) + self.b['b_final']
	    print 'hfinal', h_final.shape
            return h_final
