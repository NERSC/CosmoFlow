            ...
            with tf.Session(config=config) as sess:
                ...
                for i in range(0,int(hp.RUNPARAM['iter_test'])):
                    ...
                    if (mc.get_rank() == 0):
                        ...
                        ### make test_true_ the right length 
                        truths = np.repeat(test_true_, 64, axis=0)
                        print truths.shape
                        #print truths
                        #print test_predict_
                        np.savetxt(os.path.join(hp.Path['test_result'],'test_batch_'+str(i)+'.txt'),np.c_[truths,test_predict_])
