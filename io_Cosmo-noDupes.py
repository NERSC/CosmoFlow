...
if __name__ == '__main__':
    ...
    label_path = os.path.join('/global/cscratch1/sd/djbard/MUSIC_pyCola/egpbos-pycola-672c58551ff1/OmSiNs/fourk/','list-4000-noCiC-128from256.txt')
    labels = np.loadtxt(label_path,delimiter=',')    
       
    
    ### I want to make a list of the 3994 dirs, 8x over. 
    ### Each of these 3994 needs to be associated with 0-7, one time only! 
    ### well, I need to keep 150 dirs for val and 50 for test, so that's 3794 dirs. 
    ...
    ### Need to keep 400 records for trining and 400 for testing. 
    ### Second 2k (in twothousand-2)

    ### read in the dir/file pairs
    pairs = []
    for line in open("3800-train.txt"):
      cols = line.split(",")
      pairs.append([int(cols[0][1:]), int(cols[1][:-2]) ])
      
    #print pairs[:10]

    for i in range(130,150): ### 3800*8/64 = 475# files for training
        data = []
        label = []

        for j in range(64):
          ### I want to pull the next 64 dir/file pairs from the file. 
          thispair = pairs[(i*64)+j]
          numDirectory = thispair[0]+1000 ## because of this idiotic naming convention
          numFile = thispair[1]
          ...
          data_path = os.path.join('/global/cscratch1/sd/djbard/MUSIC_pyCola/egpbos-pycola-672c58551ff1/OmSiNs/fourk/128from256-4000/',str(dirname).rjust(3,'0'),str(numFile)+'.npy')
          #print data_path
